...
| メソッド | エンドポイント | 説明 |
| :--- | :--- | :--- |
| **GET** | `/api/v1/account/:id/status` | 口座状況（純資産・借金フラグ等）を取得 |
| **GET** | `/api/v1/account/history` | 通帳の履歴リストを取得 |
| **PATCH** | `/api/v1/account/history/print` | 記帳演出（カチャカチャ音）完了の通知 |

### 3.2 システム内部用 API（他サービス・AIエンジン向け）
| メソッド | エンドポイント | 説明 | 発生タイミング |
| :--- | :--- | :--- | :--- |
| **POST** | `/internal/v1/account/create` | **口座のみ作成（残高0）** | ユーザー登録完了直後 |
| **POST** | `/internal/v1/account/initialize` | **初期ローン100万実行** | チュートリアル開始時 |
| **POST** | `/internal/v1/account/transaction` | 資金移動（売買・利息等） | 株式売買・バッチ処理時 |
| **POST** | `/internal/v1/account/settle` | 最終精算とスコア判定 | 6日目終了時 |
